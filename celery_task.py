...
def get_config_option(section, option):
    ...
    try:
        ...
        logging.warning("Value does not exist for section {0} and option {1}".format(section, option))
    return ret
...
def encode_video(input_file_path, output_file_name):
    ...
    ffmpeg_args = ["avconv -y -i {0} -preset faster -crf 21 {1}".format(input_file_name, output_file_name)]
